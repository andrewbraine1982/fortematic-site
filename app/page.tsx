      ...
      <section style={{ padding: "80px 7vw 60px", background: "linear-gradient(120deg,#fff 0%,#f7fbff 50%,#f7f2ff 100%)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.1fr", alignItems: "center", gap: 60 }}>
          <div>
            ...
            <h1 style={{ fontSize: 82, lineHeight: 0.95, letterSpacing: -5, margin: 0, fontWeight: 900 }}>
            ...
            <div style={{ marginTop: 24, fontWeight: 800, textDecoration: "underline" }}>
              Explore the Science →
            </div>
          </div>
        ...
        <div style={{ marginTop: 48, background: "#fff", border: "1px solid #e7e7e7", borderRadius: 18, boxShadow: "0 18px 45px rgba(0,0,0,.08)", display: "grid", gridTemplateColumns: "repeat(4,1fr)" }}>
          <div style={trustBox}><strong>Clean & Safe</strong><br /><span style={muted}>No fillers. No junk.</span></div>
          <div style={trustBox}><strong>Third-party Tested</strong><br /><span style={muted}>For purity & potency.</span></div>
          <div style={trustBox}><strong>Made in USA</strong><br /><span style={muted}>GMP Certified Facility</span></div>
          <div style={trustBox}><strong>20,000+</strong><br /><span style={muted}>Happy Customers</span></div>
        </div>
      </section>
      ...
      <section style={{ padding: "90px 7vw", display: "grid", gridTemplateColumns: "1fr 1.3fr", gap: 60, alignItems: "center" }}>
        <div>
          ...
          <p style={{ fontSize: 18, lineHeight: 1.7, color: "#444" }}>
            GLP-1 is a natural hormone your body produces to help regulate appetite, food intake and metabolism.
          </p>
...
function ProductCard({ title, copy, bg, color }: { title: string; copy: string; bg: string; color: string }) {
  return (
    <div style={{ background: bg, borderRadius: 14, padding: 28, minHeight: 310 }}>
      <h3 style={{ fontSize: 24, margin: 0 }}>{title}</h3>
      <p style={{ color: "#333", lineHeight: 1.5 }}>{copy}</p>
      <div style={{ height: 150, borderRadius: 18, background: "#fff", marginTop: 20, display: "flex", alignItems: "center", justifyContent: "center", color, fontSize: 28, fontWeight: 900 }}>
        {title}
      </div>
